  ...
  return {
    ...
    link: function($scope, el, attrs) {
    	...
    	if ($scope.useNaturalModalities) {
	    	...
	    	var avgExtent = d3.extent($scope.naturalModalities, function(d){ return d.value })
        ...
        function draw(container, attribute) {
        	...
        	var svg = d3.select(container).append('svg')
				  ...
				  bars.enter().append('rect')
				      .attr('class', 'bar')
				      .attr('x', function(d) { return x(d.min) + settings.bar_spacing/2 })
				      .attr('y', function(d) { return height - y(d.count) })
				      .attr('height', function(d) { return y(d.count) })
				      .attr('width', function(d) { return Math.max(6, x(d.max) - x(d.min) - settings.bar_spacing) })
...
.directive('rankingModalitiesChart', function(
    $timeout
  ){
    return {
      restrict: 'A',
      ...
      link: function($scope, el, attrs) {
        ...
        function draw(container, attribute) {
        	...
        	var svg = d3.select(container).append('svg')
				  ...
				  // labels
				  var labels = bars.enter().append('text')
				      .attr('x', function(d) { return x(d.value) + 0.5 * x.bandwidth() - 0.5*settings.bar_spacing })
				      .attr('y', function(d) { return height - y(d.count) - 3 })
				      .text( function (d) { return d.count ? d.count : '' })
				      .attr('text-anchor', 'middle')
				      .attr('font-family', 'Quicksand, sans-serif')
...
.directive('rankingModalityLogChart', function(
    $timeout,
    $filter
  ){
    return {
        ...
        function draw(container, attribute) {
        	...
        	var svg = d3.select(container).append('svg')
				  ...
				  items.enter().append('circle')
				      ...
				      .attr('fill', settings.item_color)
			      .append('title')
			      	.text(function(d){ return d.count + ' nodes at ' + $filter('number')(d.value) })
