...
angular.module('app.components.cardModalityConnectivityBalance', [])

.directive('cardModalityConnectivityBalance', function($timeout, networkData, scalesUtils){
  return {
    restrict: 'A',
...
.directive('modalityBalanceChart', function(
  $timeout
){
  return {
    restrict: 'A',
    ...
    link: function($scope, el, attrs) {
      ...
      function redraw() {
        if ($scope.data !== undefined){
          $timeout(function(){
            el.html('');
            drawValueSkewnessDistribution(d3.select(el[0]), $scope.data, $scope.modalityValue)
          })
        }
      }
