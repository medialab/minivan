...
angular
  .module('iwanthue.paletteGenerator', [])
  ...
  .factory('paletteGenerator', function($http, $timeout) {
    ...
    ns.generate = function(
      ...
      // Default
      if (colorsCount === undefined) colorsCount = 8
      if (checkColor === undefined)
        checkColor = function(x) {
          return true
        }
      if (forceMode === undefined) forceMode = false
      ...
      if (randomSeed === undefined) {
        ...
        var intseed = d3.sum(
          ('' + randomSeed).split('').map(function(d) {
      ...
      if (forceMode) {
        ...
        return colors.map(function(lab) {
          return d3.lab(lab[0], lab[1], lab[2]).rgb()
        })
      } else {
        // K-Means Mode
        function checkColor2(lab) {
          ...
          return (
            ns.validateLab(lab) &&
            checkColor(d3.rgb(d3.lab(lab[0], lab[1], lab[2])))
          )
        }
            ...
            if (
              ...
              if (freeColorSamples.length > 0) {
                ...
                for (i = 0; i < freeColorSamples.length; i++) {
                  var distance = ns.getColorDistance(
                    freeColorSamples[i],
                    candidateKMean,
                    distanceType
                  )
    ...
    ns.diffSort = function(colorsToSort, distanceType) {
      ...
      while (colorsToSort.length > 0) {
        ...
        for (
          candidate_index = 0;
          candidate_index < colorsToSort.length;
          candidate_index++
        ) {
          var d = Infinity
    ...
    ns.validateLab = function(lab) {
      ...
      var g = xyz_rgb(-0.969266 * x + 1.8760108 * y + 0.041556 * z)
